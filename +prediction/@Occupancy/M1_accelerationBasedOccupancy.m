...
if (ts == tf) || isa(obstacle,'world.StaticObstacle')
    ...
    % (Theorem 1)
    p = geometry.addObjectDimensions(q, obstacle.length, obstacle.width);
...
elseif isa(obstacle,'world.DynamicObstacle')    
    ...
    for t = ts:dt:(tf-dt)
        ...
        q = prediction.Occupancy.accelerationOccupancyLocal(obstacle.velocity, obstacle.a_max, t, t+dt, obstacle.constraint3);
