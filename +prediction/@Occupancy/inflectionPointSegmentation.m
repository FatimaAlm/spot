...
if xiClosest < xi(1)
    ...
    if prediction.Occupancy.EXTEND_LANES_BACKWARD
        [frontBound, iBorder_frontBound] = prediction.Occupancy.constructBound(iPath_Obstacle, xiClosest, 'front', xi, shortestPath, leftBorder, rightBorder);
    else
        frontBound = [];
        iBorder_frontBound = 1;
    end
...
if xiFurthest < xi(1)
    ...
    if prediction.Occupancy.EXTEND_LANES_BACKWARD
        [finalBound, iBorder_finalBound] = prediction.Occupancy.constructBound(iPath_Obstacle, xiFurthest, 'final', xi, shortestPath, leftBorder, rightBorder);
    else
...
elseif xiFurthest > xi(end)
    ...
    if prediction.Occupancy.EXTEND_LANES_FORWARD
        ...
        finalBound = [];
        iBorder_finalBound = size(leftBorder.vertices,2);
    end
else
...
verticesLeft = leftBorder.vertices(:,iBorder_frontBound:iBorder_finalBound);
verticesRight = fliplr(rightBorder.vertices(:,iBorder_frontBound:iBorder_finalBound));
