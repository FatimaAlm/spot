function [lanelets, obstacles, egoVehicles, adjacencyGraph] = readXMLFile(filename)
...
if (numObstacles > 0)
    ...
    for i = 0:(numObstacles-1)
        ...
        if ~isempty(obstacleShapeList.item(0)) && strcmp(obstacleShapeList.item(0).getAttribute('type'), 'rectangle')
            ...
            warning('Obstacle shape is not of type rectangle.');
        end
        ...
        elseif ~isempty(obstacleTrajectoryList.item(0)) && strcmp(obstacleTrajectoryList.item(0).getAttribute('type'), 'node')
            ...
            warning('Obstacle trajectory is not of type way.');
        end
        
        % extract the type of the obstacle (by tag)
        obstacleTagList = obstacleList.item(i).getElementsByTagName('tag');
        % (insert for loop here, if obstacle has more tags)
        if ~isempty(obstacleTagList.item(0)) && strcmp(obstacleTagList.item(0).getAttribute('k'), 'type')
            obstacles(i+1).type = char(obstacleTagList.item(0).getAttribute('v'));
        else
