...
for i = iStart:(length(innerBound.distances)-1)
    
    % check if an inflection point is reached,
    % which is defined by a sign change of the curvature
    if ( sign(innerBound.curvatures(i)) ~= sign(innerBound.curvatures(i+1)) ) && ...
            ( sign(outerBound.curvatures(i)) ~= sign(outerBound.curvatures(i+1)) ) && ...
            ( sign(innerBound.curvatures(i+1)) ~= 0 )
        %TODO: also check sign(curvatures(i+2)) for more robustness?!
        
        % find the previous and next vertice point on inner bound (p)
        if (i-1) > 0
            piminus1 = innerBound.vertices(:,i-1);
        else
            piminus1 = innerBound.vertices(:,i);
        end
        piplus1 = innerBound.vertices(:,i+1);
        ...
        % % or simply just use: mu_j = outerBound.vertices(:,i);
        % % which is a quicker alternative to projection/intersection
        % %TODO: check over-approximation which will not be guaranteed?!
        ...
        if j-1 > 0
            mu_jminus1 = outerBound.vertices(:,j-1);
            hPrimeOuterBound = (mu_jminus1 - mu_j) / norm(mu_jminus1 - mu_j);
        else
            mu_jplus1 = outerBound.vertices(:,j+1);
        ...
        if (j > i) % the new inner bound (j) is ahead of the old one (i)
            ...
            % add distance beta (i.e. distance mu to mu_j)
            distance = beta;            
            % add the distances from mu_j until (i+1)
            for k = (j+1):(i+1)
                distance = distance + outerBound.distances(k);
            end
            % add all distances to path variable xi
            shortestPath.xi(end+1) = shortestPath.xi(end) + distance;
        ...
        if shortestPath.side(end) % -> left
            if sign(outerBound.curvatures(iStartNew)) < 0
...
%                     ], (i+1));
        ...
        % update shortestPath:        
        % take one step along the current inner bound by adding the distance
        % between the ith and (i+1)th vertice to path variable xi
        shortestPath.xi(end+1) = shortestPath.xi(end) + innerBound.distances(i+1);
        % the current index is i+1
        shortestPath.indexBorder(end+1) = i+1;
